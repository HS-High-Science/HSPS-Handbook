...
### Code Blue
Code Blue is the other state you will be in most often. Code Blue is active from the moment the first FFD hack announcement is played/the terminal is hacked, or if the laptop in the Warhead Control Room is bypassed. It should be noted that Code Blue is not active after the first FFD hack announcement if the FFD is locked, and Code Blue is not active after the warhead laptop has been bypassed if the Warhead is locked (Cancelled Twice). **You are required to try to stop malicious events.**
